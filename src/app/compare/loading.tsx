  ...
  return (
    <div className="animate-page-in mx-auto w-full max-w-[1200px] space-y-10 pb-16">
      <section className="cb-surface p-8">
        ...
        <div className="mt-8 h-16 max-w-3xl rounded-lg animate-shimmer" />
        <div className="mt-4 h-5 max-w-xl rounded-[100px] animate-shimmer" />
      </section>

      <section className="rounded-lg bg-[#0a0b0d] p-8 shadow-2xl shadow-black/15">
        <div className="h-5 w-44 rounded-[100px] bg-[#16181c]" />
        <div className="mt-6 h-10 max-w-lg rounded-lg bg-[#16181c]" />
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {[0, 1, 2].map((item) => (
            <div key={item} className="h-32 rounded-lg bg-[#16181c]" />
          ))}
        </div>
      </section>
        ...
        {[0, 1, 2, 3].map((item) => (
          <div key={item} className="cb-card p-6">
            ...
            <div className="mt-4 grid grid-cols-4 gap-3">
              {[0, 1, 2, 3].map((metric) => (
                <div key={metric} className="h-12 rounded-lg animate-shimmer" />
              ))}
            </div>
          </div>
        ))}
