        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <caption className="sr-only">
              Saved product watches with target price, trust score, markets, and audit action.
            </caption>
            <thead className="bg-[#f7f7f7] text-xs text-[#5b616e]">
              <tr>
                ...
                <th scope="col" className="px-6 py-4 font-semibold">Markets</th>
                <th scope="col" className="px-6 py-4 font-semibold">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#dee1e6]">
              {savedAudits.map((item) => (
                <tr key={item.product} className="cb-table-row">
                  <th scope="row" className="px-6 py-4 text-left font-semibold text-[#0a0b0d]">
                    {item.product}
                  ...
                  <td className="px-6 py-4">
                    <Link href={`/compare?q=${encodeURIComponent(item.product)}`} className="cb-action rounded-full font-semibold text-[#0052ff] hover:text-[#003ecc]">
                      Re-run audit
                    </Link>
                  </td>
                </tr>
